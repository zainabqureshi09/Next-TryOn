  ...
  return (
    <Link href={`/product/${id}`} className="group" ref={ref as any}>
      <div className="border rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 relative">
        {isOut && (
          <span className="absolute top-3 left-3 z-10 rounded bg-red-600 px-2 py-1 text-xs font-semibold text-white">Out of stock</span>
        )}
        {isLow && (
          <span className="absolute top-3 left-3 z-10 rounded bg-amber-500 px-2 py-1 text-xs font-semibold text-white">Low stock</span>
        )}
        <Image
          src={image}
          alt={name}
