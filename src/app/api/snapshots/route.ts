...
export async function GET() {
  ...
  await dbConnect();
  const snaps = await Snapshot.find({ userEmail: session.user.email }).sort({ createdAt: -1 }).lean();
...
export async function POST(req: Request) {
  ...
  if (!cloudName || !uploadPreset) return NextResponse.json({ error: "Cloudinary not configured" }, { status: 500 });
  ...
  await dbConnect();
  const created = await Snapshot.create({ userEmail: session.user.email, url: json.secure_url });
